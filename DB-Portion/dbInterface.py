...
def archiveTweets(location = 'remote'):
    ...
    cursor = cnxn.cursor()
    cursor.execute("""INSERT INTO arc_tweetTest(dateTweet, userName, tweetContent) SELECT dateTweet, userName, tweetContent from tweetTest""")
...
#archiveTweets('local')
#pushSAInfo('finished.csv', 'local')
#archiveTweets('local')
#pushTweets('twitTweets.csv', 'local')
